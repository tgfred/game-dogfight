...
def init_game(plus):
    ...
    Main.texture_hud_plot_aircraft = plus.LoadTexture(
        "assets/sprites/plot.png")
    Main.texture_hud_plot_missile = plus.LoadTexture(
        "assets/sprites/plot_missile.png")
    Main.texture_hud_plot_ship = plus.LoadTexture(
        "assets/sprites/plot_ship.png")
...
def load_scene_parameters(file_name="assets/scripts/scene_parameters.json"):
    ...
    if json_script != "":
        ...
        Main.ligth_sun.GetLight().SetDiffuseColor(
            list_to_color(script_parameters["sunlight_color"]))
        Main.ligth_sky.GetLight().SetDiffuseColor(
            list_to_color(script_parameters["skylight_color"]))
        environment.SetAmbientColor(list_to_color(
            script_parameters["ambient_color"]))
...
def load_game_parameters(file_name="assets/scripts/dogfight.json"):
    ...
    if json_script != "":
        script_parameters = json.loads(json_script)
        set_p1_missiles_smoke_color(list_to_color(
            script_parameters["p1_missiles_smoke_color"]))
        set_p2_missiles_smoke_color(list_to_color(
            script_parameters["p2_missiles_smoke_color"]))
...
def post_processes_load_parameters(file_name="assets/scripts/post_render.json"):
    ...
    if json_script != "":
        ...
        Main.MotionBlur_postProcess.SetBlurRadius(
            script_parameters["mb_blur_radius"])
        Main.MotionBlur_postProcess.SetExposure(
            script_parameters["mb_exposure"])
        Main.MotionBlur_postProcess.SetSampleCount(
            int(script_parameters["mb_sample_count"]))
...
def init_scene(plus):
    ...
    Main.camera = plus.AddCamera(
        Main.scene, hg.Matrix4.TranslationMatrix(hg.Vector3(0, 10, -10)))
    ...
    Main.satellite_camera = plus.AddCamera(
        Main.scene, hg.Matrix4.TranslationMatrix(hg.Vector3(0, 1000, 0)))
    setup_satellite_camera(Main.satellite_camera)

    # ---- Clouds:
    json_script = hg.GetFilesystem().FileToString(
        "assets/scripts/clouds_parameters.json")
    if json_script != "":
        clouds_parameters = json.loads(json_script)
        Main.clouds = Clouds(plus, Main.scene, Main.scene.GetNode(
            "Sun"), Main.resolution, clouds_parameters)
    ...
    Main.sea_render_script = hg.RenderScript(
    ...
    Main.sea_render.update_render_script(
        Main.scene, Main.resolution, hg.time_to_sec_f(plus.GetClock()))
    Main.scene.AddComponent(Main.sea_render_script)

    Main.water_reflection = WaterReflection(
        plus, Main.scene, Main.resolution, Main.resolution.x / 4)
...
def init_lights(plus):
    ...
    Main.ligth_sun.GetLight().SetShadow(
...
def gui_interface_scene(scene, fps):
    ...
    if hg.ImGuiBegin("Scene Settings"):
        ...
        d, f = hg.ImGuiCheckbox(
            "Display collisions shapes", Main.show_debug_displays)
        if d:
            Main.show_debug_displays = f
            scene.GetPhysicSystem().SetDebugVisuals(Main.show_debug_displays)

        d, f = hg.ImGuiCheckbox("Volumetric clouds",
                                Main.render_volumetric_clouds)
        if d:
...
def gui_interface_game(scene):
    if hg.ImGuiBegin("Game Settings"):
        ...
        if f:
            set_p1_missiles_smoke_color(c)

        f, c = hg.ImGuiColorEdit(
            "P2 Missiles smoke color", Main.p2_missiles_smoke_color)
        if f:
            set_p2_missiles_smoke_color(c)
        ...
        hg.ImGuiSeparator()
        d, f = hg.ImGuiSliderFloat(
            "Radial blur strength", Main.radial_blur_strength, 0, 1)
        if d:
            Main.radial_blur_strength = f
        d, f = hg.ImGuiSliderFloat(
            "Deceleration blur strength", Main.deceleration_blur_strength, 0, 1)
        if d:
            Main.deceleration_blur_strength = f
        d, f = hg.ImGuiSliderFloat(
            "Acceleration blur strength", Main.acceleration_blur_strength, 0, 1)
        if d:
            Main.acceleration_blur_strength = f
...
def gui_interface_sea_render(sea_render: SeaRender, scene, fps):
    if hg.ImGuiBegin("Sea & Sky render Settings"):
        ...
        d, f = hg.ImGuiCheckbox(
            "Water reflection", sea_render.render_scene_reflection)
        if d:
            sea_render.render_scene_reflection = f

        d, f = hg.ImGuiSliderFloat(
            "Texture North intensity", sea_render.tex_sky_N_intensity, 0, 1)
        if d:
            sea_render.tex_sky_N_intensity = f
        d, f = hg.ImGuiSliderFloat(
            "Zenith falloff", sea_render.zenith_falloff, 1, 100)
        if d:
            sea_render.zenith_falloff = f

        f, c = hg.ImGuiColorEdit("Zenith color", sea_render.zenith_color)
        if f:
            sea_render.zenith_color = c
        f, c = hg.ImGuiColorEdit("Horizon color", sea_render.horizon_N_color)
        if f:
            sea_render.horizon_N_color = c

        f, c = hg.ImGuiColorEdit("Water color", sea_render.sea_color)
        if f:
            sea_render.sea_color = c
        f, c = hg.ImGuiColorEdit(
            "Horizon Water color", sea_render.horizon_S_color)
        if f:
            sea_render.horizon_S_color = c
        f, c = hg.ImGuiColorEdit("Horizon line color",
                                 sea_render.horizon_line_color)
        if f:
            sea_render.horizon_line_color = c

        hg.ImGuiText("3/4 horizon line size: " +
                     str(sea_render.horizon_line_size))

        f, d = hg.ImGuiCheckbox("Sea texture filtering",
                                bool(sea_render.sea_filtering))
        if f:
            sea_render.sea_filtering = int(d)
        hg.ImGuiText("5/6 max filter samples: " +
                     str(sea_render.max_filter_samples))
        hg.ImGuiText("7/8 filter precision: " +
                     str(sea_render.filter_precision))
        ...
        d, f = hg.ImGuiSliderFloat(
            "Sea reflection", sea_render.sea_reflection, 0, 1)
        if d:
            sea_render.sea_reflection = f
        d, f = hg.ImGuiSliderFloat(
            "Reflect offset", Main.sea_render.reflect_offset, 1, 1000)
        if d:
            Main.sea_render.reflect_offset = f
...
def gui_clouds(scene: hg.Scene, cloud: Clouds):
    ...
    if hg.ImGuiBegin("Clouds Settings"):
        ...
        d, f = hg.ImGuiSliderFloat(
            "Clouds scale x", cloud.map_scale.x, 100, 10000)
        if d:
            cloud.set_map_scale_x(f)
        d, f = hg.ImGuiSliderFloat(
            "Clouds scale z", cloud.map_scale.y, 100, 10000)
        if d:
        ...
        d, f = hg.ImGuiSliderFloat(
            "Clouds altitude", clouds_altitude, 100, 10000)
        if d:
            clouds_altitude = f
            if link_altitudes:
        ...
        d, f = hg.ImGuiSliderFloat(
            "Clouds morph level", clouds_morph_level, 0, 1)
        if d:
...
def gui_layer(layer: CloudsLayer):
    ...
    d, f = hg.ImGuiSliderFloat(
        nm + " particles rotation speed", layer.particles_rot_speed, -10, 10)
    if d:
        layer.set_particles_rot_speed(f)

    d, f = hg.ImGuiSliderFloat(
        nm + " particles morph level", layer.morph_level, -1, 1)
    if d:
        layer.morph_level = f

    d, f = hg.ImGuiSliderFloat(
        nm + " Absorption factor", layer.absorption * 100, 0.01, 10)
    if d:
        layer.set_absorption(f / 100)

    d, f = hg.ImGuiSliderFloat(
        nm + " Altitude floor", layer.altitude_floor, -2, 2)
    if d:
        layer.set_altitude_floor(f)
    ...
    d, f = hg.ImGuiSliderFloat(
        nm + " Altitude falloff", layer.altitude_falloff, 0.1, 100)
    if d:
        layer.set_altitude_falloff(f)

    d, f = hg.ImGuiSliderFloat(
        nm + " Particles min scale", layer.particles_scale_range.x, 1, 5000)
    if d:
        layer.set_particles_min_scale(f)
    d, f = hg.ImGuiSliderFloat(
        nm + " Particles max scale", layer.particles_scale_range.y, 1, 5000)
    if d:
        layer.set_particles_max_scale(f)
    d, f = hg.ImGuiSliderFloat(
        nm + " Alpha threshold", layer.alpha_threshold, 0, 1)
    if d:
        layer.alpha_threshold = f

    d, f = hg.ImGuiSliderFloat(
        nm + " Scale falloff", layer.scale_falloff, 1, 10)
    if d:
        layer.scale_falloff = f

    d, f = hg.ImGuiSliderFloat(
        nm + " Alpha scale falloff", layer.alpha_scale_falloff, 1, 10)
    if d:
        layer.alpha_scale_falloff = f

    d, f = hg.ImGuiSliderFloat(
        nm + " Perturbation", layer.perturbation, 0, 200)
    if d:
    ...
    if d:
        layer.tile_size = f
    d, f = hg.ImGuiSliderFloat(
        nm + " Distance min", layer.distance_min, 0, 5000)
    if d:
        layer.set_distance_min(f)
    d, f = hg.ImGuiSliderFloat(
        nm + " Distance max", layer.distance_max, 100, 5000)
    if d:
    ...
    if d:
        layer.margin = f
    d, f = hg.ImGuiSliderFloat(
        nm + " Focal margin", layer.focal_margin, 0.5, 2)
    if d:
...
def gui_post_rendering():
    if hg.ImGuiBegin("Post-rendering Settings"):
        ...
        if d:
            Main.HSL_postProcess.SetH(f)
        d, f = hg.ImGuiSliderFloat(
            "Saturation", Main.HSL_postProcess.GetS(), 0, 1)
        if d:
            Main.HSL_postProcess.SetS(f)
        d, f = hg.ImGuiSliderFloat(
            "Luminance", Main.HSL_postProcess.GetL(), 0, 1)
        if d:
            Main.HSL_postProcess.SetL(f)
        ...
        if Main.flag_MotionBlur:
            ...
            if d:
                pp.SetExposure(f)
            d, f = hg.ImGuiSliderInt(
                "SampleCount", pp.GetSampleCount(), 1, 100)
            if d:
                pp.SetSampleCount(f)
...
def autopilot_controller(aircraft: Aircraft):
    if hg.ImGuiBegin("Auto pilot"):
        ...
        d, f = hg.ImGuiSliderFloat(
            "Pitch", aircraft.autopilot_pitch_attitude, -180, 180)
        if d:
            aircraft.set_autopilot_pitch_attitude(f)
        d, f = hg.ImGuiSliderFloat(
            "Roll", aircraft.autopilot_roll_attitude, -180, 180)
        if d:
            aircraft.set_autopilot_roll_attitude(f)
        d, f = hg.ImGuiSliderFloat("Cap", aircraft.autopilot_cap, 0, 360)
        if d:
            aircraft.set_autopilot_cap(f)
        d, f = hg.ImGuiSliderFloat(
            "Altitude", aircraft.autopilot_altitude, 0, 10000)
        if d:
            aircraft.set_autopilot_altitude(f)
    hg.ImGuiEnd()


def control_aircraft_paddle(dts, aircraft: Aircraft):

    if Main.controller is None:
    ...
    if v != 0:
        aircraft.set_thrust_level(aircraft.thrust_level + v * 0.01)
    if ct.IsButtonDown(hg.ButtonCrossUp):
        aircraft.set_brake_level(aircraft.brake_level + 0.01)
    if ct.IsButtonDown(hg.ButtonCrossDown):
        aircraft.set_brake_level(aircraft.brake_level - 0.01)
    if ct.IsButtonDown(hg.ButtonCrossLeft):
        aircraft.set_flaps_level(aircraft.flaps_level - 0.01)
    if ct.IsButtonDown(hg.ButtonCrossRight):
        aircraft.set_flaps_level(aircraft.flaps_level + 0.01)
...
def start_phase(plus, delta_t):
    ...
    if Main.fading_cptr >= fade_in_delay:
        ...
        # Function
        plus.Text2D(x, y / 900 * Main.resolution.y, "Thrust level",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 20) / 900 * Main.resolution.y, "Pitch",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 40) / 900 * Main.resolution.y, "Roll",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 60) / 900 * Main.resolution.y, "SYaw",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 80) / 900 * Main.resolution.y, "Gun",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 100) / 900 * Main.resolution.y,
                    "Missiles", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 120) / 900 * Main.resolution.y,
                    "Target selection", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 140) / 900 * Main.resolution.y,
                    "Brake", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 160) / 900 * Main.resolution.y,
                    "Flaps", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 180) / 900 * Main.resolution.y, "Post combustion (only thrust=100%)", s * Main.resolution.y,
                    c, Main.hud_font)
        plus.Text2D(x, (y - 200) / 900 * Main.resolution.y,
                    "Reset game", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 220) / 900 * Main.resolution.y,
                    "Set View", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 240) / 900 * Main.resolution.y,
                    "Satellite Zoom", s * Main.resolution.y, c, Main.hud_font)
        # Keyboard
        c = hg.Color.White
        x = 815 / 1600 * Main.resolution.x
        plus.Text2D(x, y / 900 * Main.resolution.y, "Home / End",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 20) / 900 * Main.resolution.y,
                    "Up / Down", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 40) / 900 * Main.resolution.y,
                    "Right / Left", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 60) / 900 * Main.resolution.y,
                    "Suppr / Page down", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 80) / 900 * Main.resolution.y, "ENTER",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 100) / 900 * Main.resolution.y, "F1",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 120) / 900 * Main.resolution.y, "T",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 140) / 900 * Main.resolution.y,
                    "B / N", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 160) / 900 * Main.resolution.y,
                    "C / V", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 180) / 900 * Main.resolution.y,
                    "Space", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 200) / 900 * Main.resolution.y,
                    "Backspace", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 220) / 900 * Main.resolution.y,
                    "2/4/8/6/5", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 240) / 900 * Main.resolution.y,
                    "Insert / Page Up", s * Main.resolution.y, c, Main.hud_font)

        # Paddle
        x = 990 / 1600 * Main.resolution.x
        plus.Text2D(x, y / 900 * Main.resolution.y, "Right pad up / down",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 20) / 900 * Main.resolution.y,
                    "Left pad", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 40) / 900 * Main.resolution.y,
                    "Left pad", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 60) / 900 * Main.resolution.y,
                    "RT / LT", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 80) / 900 * Main.resolution.y, "A",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 100) / 900 * Main.resolution.y, "X",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 120) / 900 * Main.resolution.y, "Y",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 140) / 900 * Main.resolution.y,
                    "Cross Up / Down", s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 160) / 900 * Main.resolution.y, "Cross Right / LEft", s * Main.resolution.y, c,
                    Main.hud_font)
        plus.Text2D(x, (y - 180) / 900 * Main.resolution.y, "B",
                    s * Main.resolution.y, c, Main.hud_font)
        plus.Text2D(x, (y - 200) / 900 * Main.resolution.y, "Back",
                    s * Main.resolution.y, c, Main.hud_font)

        start = False
        if Main.controller is not None:
            # if Main.controller.WasButtonPressed(hg.ButtonStart):
            # 直接開始
            start = True
...
def update_radial_post_process(acceleration):
    if acceleration < 0:
        Main.RadialBlur_postProcess.SetStrength(
            Main.radial_blur_strength*pow(min(1, abs(acceleration*Main.deceleration_blur_strength)), 2))
    else:
        Main.RadialBlur_postProcess.SetStrength(
            Main.radial_blur_strength*pow(min(1, abs(acceleration*Main.acceleration_blur_strength)), 4))
...
def main_phase(plus, delta_t):
    ...
    if Main.satellite_view:
        update_satellite_camera(Main.satellite_camera,
                                Main.resolution.x / Main.resolution.y, dts)
    ...
    back = False
    if Main.controller is not None:
        if Main.controller.WasButtonPressed(hg.ButtonBack):
            back = True
...
def end_phase(plus, delta_t):
    ...
    if Main.fading_cptr < fade_in_delay:
        Main.fading_cptr = min(fade_in_delay, Main.fading_cptr + dts)
        Main.HSL_postProcess.SetS(
            (1 - Main.fading_cptr / fade_in_delay) * Main.fading_start_saturation)
...
if smr_ok == "ok":
    ...
    plus.RenderInit(int(Main.resolution.x), int(
        Main.resolution.y), Main.antialiasing, Main.screenMode)
