...
class Missile(Destroyable_Machine):
    ...
    def __init__(self, name, nationality, plus, scene, audio, missile_file_name, smoke_file_name_prefix,
        ...
        Destroyable_Machine.__init__(
            self, nd, Destroyable_Machine.TYPE_MISSILE, nationality)
        ...
        self.explode.set_rot_range(radians(20), radians(
            50), radians(10), radians(45), radians(5), radians(15))
    ...
    def update_kinetics(self, scene, dts):
        if self.activated:
            ...
            if not self.wreck:
                ...
                q = hg.Vector3(pow(spdX.Len(), 2), pow(spdY.Len(), 2), pow(
                    spdZ.Len(), 2)) * 0.5 * self.air_density

                # Drag force:
                F_drag = spdX.Normalized() * q.x * self.drag_coeff.x + spdY.Normalized() * \
                    q.y * self.drag_coeff.y + spdZ.Normalized() * q.z * self.drag_coeff.z
                ...
                if self.target is not None:
                    ...
                    target_dir = (target_node.GetTransform(
                    ).GetPosition() - pos).Normalized()
                ...
                if self.target is not None:
                    spd = self.v_move.Len()
                    hit, impact = scene.GetPhysicSystem().Raycast(
                        pos, self.v_move.Normalized(), 0x255, spd)
                    if hit:
                ...
                self.audio.SetChannelState(
                    self.turbine_channel, self.turbine_settings)

            else:
                pos = self.parent_node.GetTransform().GetPosition()
                self.explode.update_kinetics(
                    pos, hg.Vector3.Front, self.v_move, hg.Vector3.Up, dts)
    ...
    def start_explosion(self):
        if not self.wreck:
            self.audio.Stop(self.turbine_channel)
            self.explosion_settings.volume = min(
                1, self.turbine_settings.volume * 2)
            self.audio.Start(self.audio.LoadSound(
                "assets/sfx/missile_explosion.wav"), self.explosion_settings)
            self.wreck = True
...
class MachineGun(ParticlesEngine):
    def __init__(self, name, plus, scene):
        ...
        for i in range(self.num_particles):
            ...
            fb.colors = [hg.Color(1., 1., 1., 1), hg.Color(
                1., .5, 0.25, 0.25), hg.Color(0.1, 0., 0., 0.)]
            fb.set_rot_range(radians(20), radians(50), radians(
                10), radians(45), radians(5), radians(15))
    ...
    def update_kinetics(self, scene, targets, position: hg.Vector3, direction: hg.Vector3, v0: hg.Vector3,
        ...
        ParticlesEngine.update_kinetics(
            self, position, direction, v0, axisY, dts)
        for i in range(self.num_particles):
            ...
            if bullet.node.GetEnabled():
                ...
                if pos_fb.y < 1:
                    ...
                    self.strike(i)
                hit, impact = scene.GetPhysicSystem().Raycast(
                    pos, bullet.v_move.Normalized(), 0x255, spd)
                if hit:
            ...
            if not fb.end and fb.flow > 0:
                fb.update_kinetics(pos_fb, hg.Vector3.Front,
                                   bullet.v_move, hg.Vector3.Up, dts)
...
class Aircraft(Destroyable_Machine):
    ...
    def __init__(self, name, nationality, id_string, plus, scene, start_position: hg.Vector3,
        ...
        self.wing_l = scene.GetNode(
            "dummy_" + id_string + "_configurable_wing_l")
        self.wing_r = scene.GetNode(
            "dummy_" + id_string + "_configurable_wing_r")
        self.aileron_l = scene.GetNode("dummy_" + id_string + "_aileron_l")
        self.aileron_r = scene.GetNode("dummy_" + id_string + "_aileron_r")
        self.elevator_l = scene.GetNode(
            "dummy_" + id_string + "_elevator_changepitch_l")
        self.elevator_r = scene.GetNode(
            "dummy_" + id_string + "_elevator_changepitch_r")
        ...
        self.angular_frictions = hg.Vector3(
            0.000175, 0.000125, 0.000275)  # pitch, yaw, roll
        ...
        # collisions:
        self.rigid, self.rigid_wing_r, self.rigid_wing_l, self.collisions = self.get_collisions(
            scene)
        ...
        self.targets = []
        self.target_id = 0
        self.target_lock_range = hg.Vector2(
            100, 3000)  # Target lock distance range
        # Target lock delay in lock range
        self.target_lock_delay = hg.Vector2(1, 5)
        ...
        self.engines_position = [hg.Vector3(
            1.56887, -0.14824, -6.8), hg.Vector3(-1.56887, -0.14824, -6.8)]
    ...
    def create_post_combustion_particles(self, plus, scene, engine_name):
        ...
        pc.colors = [hg.Color(1., 1., 1., 1), hg.Color(1., 0.9, 0.7, 0.5), hg.Color(0.9, 0.7, 0.1, 0.25),
                     hg.Color(0.9, 0.5, 0., 0.), hg.Color(
                         0.85, 0.5, 0., 0.25), hg.Color(0.8, 0.4, 0., 0.15),
                     hg.Color(0.8, 0.1, 0.1, 0.05), hg.Color(0.5, 0., 0., 0.)]
        pc.set_rot_range(radians(1200), radians(2200), radians(
            1420), radians(1520), radians(1123), radians(5120))
    ...
    def get_missiles_slots(self, scene):
        ...
        # return slots
        for i in range(4):
            nd = scene.GetNode("dummy_" + self.id_string +
                               "_slot." + str(i + 1))
            # scene.RemoveNode(nd)
            slots.append(nd.GetTransform().GetPosition())
        return slots
    ...
    def set_health_level(self, value):
        ...
        if self.health_level < 1:
            ...
            self.smoke.flow = 0
        self.smoke.delay_range = hg.Vector2(
            1, 10) * pow(1 - self.health_level, 3)
        self.smoke.scale_range = hg.Vector2(
            0.1, 5) * pow(1 - self.health_level, 3)
    ...
    def update_mobile_parts(self):
        self.elevator_l.GetTransform().SetRotation(
            hg.Vector3(-self.parts_angles.x * self.angular_levels.x, 0, 0))
        self.elevator_r.GetTransform().SetRotation(
            hg.Vector3(-self.parts_angles.x * self.angular_levels.x, 0, 0))

        rot_l, rot_r = self.rudder_l.GetTransform().GetRotation(
        ), self.rudder_r.GetTransform().GetRotation()
        ...
        rot_l, rot_r = self.aileron_l.GetTransform().GetRotation(
        ), self.aileron_r.GetTransform().GetRotation()
    ...
    def update_kinetics(self, scene, dts):
        if self.activated:
            ...
            self.horizontal_aX = hg.Cross(
                hg.Vector3.Up, self.horizontal_aZ) * self.y_dir
            # ! It's not an orthogonal repere !
            self.horizontal_aY = hg.Cross(aZ, self.horizontal_aX)

            self.pitch_attitude = degrees(
                acos(max(-1, min(1, hg.Dot(self.horizontal_aZ, aZ)))))
            if aZ.y < 0:
                self.pitch_attitude *= -1

            self.roll_attitude = degrees(
                acos(max(-1, min(1, hg.Dot(self.horizontal_aX, aX)))))
            if aX.y < 0:
            ...
            self.set_wings_level(max(min(
                (frontal_speed * 3.6 - self.wings_thresholds.x) /
                (self.wings_thresholds.y - self.wings_thresholds.x),
                1), 0))
            ...
            # Dynamic pressure:
            q = hg.Vector3(pow(spdX.Len(), 2), pow(spdY.Len(), 2),
                           pow(spdZ.Len(), 2)) * 0.5 * self.air_density

            # F Lift
            F_lift = aY * q.z * (self.wings_lift +
                                 self.flaps_level * self.flaps_lift)
            ...
            # Total
            self.v_move += ((F_thrust + F_lift - F_drag) *
                            self.health_level + self.F_gravity) * dts
            ...
            # Angular damping:
            gaussian = exp(-pow(frontal_speed * 3.6 *
                                3 / self.speed_ceiling, 2) / 2)
            ...
            if self.flag_easy_steering or self.autopilot_activated:
                ...
                zl = min(1, abs(self.angular_levels.z +
                                self.angular_levels.x + self.angular_levels.y))
                roll_m += (easy_roll_stab * (1 - zl) + easy_turn_m_yaw) * \
                    q.z * self.angular_frictions.y * gaussian
            ...
            # Rotation matrix:
            rot_mat = MathsSupp.rotate_matrix(
                mat.GetRotationMatrix(), axis_rot, moment_speed * dts)
            self.parent_node.GetTransform().SetRotationMatrix(rot_mat)
    ...
    def update_target_lock(self, dts):
        if self.target_id > 0:
            ...
            if target.wreck or not target.activated:
                ...
                if self.target_id == 0:
                    return
            t_pos = self.targets[self.target_id -
                                 1].get_parent_node().GetTransform().GetPosition()
            mat = self.parent_node.GetTransform().GetWorld()
            dir = mat.GetZ()
            v = t_pos - mat.GetTranslation()
            self.target_cap = self.calculate_cap(
                (v * hg.Vector3(1, 0, 1)).Normalized())
            self.target_altitude = t_pos.y
            self.target_distance = v.Len()
            t_dir = v.Normalized()
            self.target_angle = degrees(
                acos(max(-1, min(1, hg.Dot(dir, t_dir)))))
            if self.target_angle < 15 and self.target_lock_range.x < self.target_distance < self.target_lock_range.y:
                ...
                delay = self.target_lock_delay.x + t * \
                    (self.target_lock_delay.y - self.target_lock_delay.x)
                self.target_out_of_range = False
...
class AircraftSFX:
    ...
    def start_engine(self, main):
        self.turbine_settings.volume = 0
        self.turbine_settings.pitch = 1  # 1
        self.air_settings.volume = 0
        self.pc_settings.volume = 0
        self.air_chan = main.audio.Start(main.audio.LoadSound(
            "assets/sfx/air.wav"), self.air_settings)
        self.turbine_chan = main.audio.Start(main.audio.LoadSound(
            "assets/sfx/turbine.wav"), self.turbine_settings)
        self.pc_chan = main.audio.Start(main.audio.LoadSound(
            "assets/sfx/post_combustion.wav"), self.pc_settings)
        self.start = False
        self.pc_started = False
    ...
    def stop_engine(self, main):
        ...
        self.wind_chan = main.audio.Start(main.audio.LoadSound(
            "assets/sfx/wind.wav"), self.wind_settings)
    ...
    def update_sfx(self, main, dts):
        ...
        if self.aircraft.wreck and not self.exploded:
            ...
            main.audio.Start(main.audio.LoadSound(
                "assets/sfx/explosion.wav"), self.explosion_settings)
            self.exploded = True
        ...
        if self.start:
            if self.aircraft.thrust_level <= 0:
                ...
                self.air_settings.volume = (
                    0.1 + self.aircraft.thrust_level * 0.9) * level

                main.audio.SetChannelState(
                    self.turbine_chan, self.turbine_settings)
                ...
                if self.aircraft.post_combution:
                    ...
                    if not self.pc_stopped:
                        self.pc_started = False
                        self.pc_settings.volume = (
                            1 - self.pc_cptr / self.pc_stop_delay) * level
                        main.audio.SetChannelState(
                            self.pc_chan, self.pc_settings)
                        self.pc_cptr += dts
...
class Carrier(Destroyable_Machine):
    def __init__(self, name, nationality, plus, scene):
        ...
        self.aircraft_start_point = scene.GetNode(
            "carrier_aircraft_start_point")
        self.aircraft_start_point.RemoveComponent(
            self.aircraft_start_point.GetObject())
